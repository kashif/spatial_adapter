...
describe "Spatially-enabled Models" do
  ...
  describe "inserting records" do
    ...
    it 'should save 3D Point (with Z coord) objects' do
      ...
      @connection.should_receive(:insert_sql).with(Regexp.new(GeometryFactory.pointz.as_hex_wkb), anything(), anything(), anything(), anything())
      model.save.should == true
    end
  
    it 'should save 3D Point (with M coord) objects' do
      model = PointmModel.new(:extra => 'test', :geom => GeometryFactory.pointm)
      @connection.should_receive(:insert_sql).with(Regexp.new(GeometryFactory.pointm.as_hex_wkb), anything(), anything(), anything(), anything())
      model.save.should == true
    end
  
    it 'should save 4D Point objects' do
      model = Point4Model.new(:extra => 'test', :geom => GeometryFactory.point4)
      @connection.should_receive(:insert_sql).with(Regexp.new(GeometryFactory.point4.as_hex_wkb), anything(), anything(), anything(), anything())
      model.save.should == true
    end
  end
